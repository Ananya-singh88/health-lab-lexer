...
const CategoryDetail = () => {
  ...
  const primaryMetric = getPrimaryMetric(categoryId || "");
  
  useEffect(() => {
    // Load data for this category
    const categoryMetrics = getCategoryMetrics(categoryId || "");
    setMetrics(categoryMetrics);
    
    const categoryRecommendations = getCategoryRecommendations(categoryId || "");
  ...
  // Create trend data for the primary metric
  const trendData = [
    { date: '2023-01-01', value: getCategoryTrendValue(categoryId || "", 0) },
    { date: '2023-03-15', value: getCategoryTrendValue(categoryId || "", 1) },
    { date: '2023-06-30', value: getCategoryTrendValue(categoryId || "", 2) },
    { date: '2023-09-15', value: getCategoryTrendValue(categoryId || "", 3) }
  ];
  
  // Generate bell curve data
  const bellCurveData = generateBellCurveData(
    getCategoryBellCurveParams(categoryId || "").mean,
    getCategoryBellCurveParams(categoryId || "").stdDev,
    getCategoryBellCurveParams(categoryId || "").userValue
  );
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <TrendChart 
                  ...
                  data={trendData}
                  unit={getPrimaryMetricUnit(categoryId || "")}
                  dataKey="value"
                />
              </div>
              ...
              <Card>
                ...
                <CardContent>
                  <BellCurveChart
                    ...
                    data={bellCurveData}
                    userValue={getCategoryBellCurveParams(categoryId || "").userValue}
                    unit={getPrimaryMetricUnit(categoryId || "")}
                    referenceMin={getCategoryBellCurveParams(categoryId || "").referenceMin}
                    referenceMax={getCategoryBellCurveParams(categoryId || "").referenceMax}
                  />
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span>{getCategoryRiskFactor(categoryId || "", 0)}</span>
                      <span className="text-green-500 font-medium">Low</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>{getCategoryRiskFactor(categoryId || "", 1)}</span>
                      <span className="text-green-500 font-medium">Low</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>{getCategoryRiskFactor(categoryId || "", 2)}</span>
...
function getCategoryRiskFactor(category: string, index: number): string {
  ...
  switch(category) {
    case "diabetes": return diabetesRisks[index];
    case "heart": return heartRisks[index];
    case "kidney": return kidneyRisks[index];
    case "thyroid": return thyroidRisks[index];
